...
export class VenueSecondInningsScoreStatistic extends BaseMatchStatisticComputer {
    private static readonly TYPE = MatchStatisticType.create(
        "venue_second_innings_score",
        "Second Innings Score",
        "Second innings total at venue",
        EntityType.VENUE,
    );

    public getType(): MatchStatisticType {
        return VenueSecondInningsScoreStatistic.TYPE;
    }

    public compute(match: Match): MatchStatistic[] {
        const inning = match.getInnings().find((entry) => entry.getInningNumber() === 2);
